...
namespace SocketRocketBinding
{
	...
	interface SRWebSocketDelegate
	{
		...
		void WebSocketDidReceiveMessage(SRWebSocket webSocket, NSObject message);
		...
		void WebSocketDidFailWithError(SRWebSocket webSocket, NSError error);
		...
		void WebSocketDidClose(SRWebSocket webSocket, nint code, string reason, bool wasClean);

		// @optional -(void)webSocket:(SRWebSocket *)webSocket didReceivePong:(NSData *)pongPayload;
		[Export("webSocket:didReceivePong:")]
		void WebSocketDidReceivePong(SRWebSocket webSocket, NSData pongPayload);
